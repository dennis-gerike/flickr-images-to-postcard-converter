...
Then('the final image should have no horizontal margin', async function () {
    const referenceImagePath = `${getFixturesFolderPath()}/reference-images/${process.env[EnvironmentVariables.MEDIA_ID]}-${process.env[EnvironmentVariables.ASPECT_RATIO] ?? 1.5}-no-h-margin.jpg`
